...
function handleForwardAction(ctx, msg, data, currentWs) {
    ...
    // 延迟关闭连接
    setTimeout(() => { if (currentWs.readyState === 1) currentWs.close(1000, "FINISH"); }, 4000);
}
/**
 * 内部辅助：处理群成员数据解析
 */
...
cmdBatchPlace.solve = (ctx, msg, cmdArgs) => {
    ...
    let places = JSON.parse(ext.storageGet("available_places") || "{}");
    const items = arg.replace(/，/g, "/").split("/");
    items.forEach(item => {
...
function cleanupConflictsAndNotify(platform, toid, toname, day, time, ctx, msg) {
    ...
    for (let req of allAppointments) {
      ...
      if (req.groupRef && a_MultiGroupRequest[req.groupRef]) {
        ...
        if (req.toname === toname && group.targetList[toname] === null) {
          ...
          // ❌ 不加入 updatedAppointments → 只移除这个人的请求
    ...
    for (let n of notifyRefused) {
      const targetGroupId = a_private_group[platform]?.[n.toname]?.[1];
      if (!targetGroupId) {
        console.log(`[❗️通知失败] 找不到 ${n.toname} 的绑定群，跳过通知`);
        continue;
      }
  
      console.log(`[🧪 DEBUG] 通知发起人 QQ:${n.sendid} → 群: ${targetGroupId}`);
  
      const msg2 = seal.newMessage();
      msg2.messageType = "group";
      msg2.sender = {};
      msg2.sender.userId = `${platform}:${n.sendid}`;
      msg2.groupId = `${platform}-Group:${targetGroupId}`;
  
      const ctx2 = seal.createTempCtx(ctx.endPoint, msg2);
      const notice = `📜 ${n.toname} 恕难从命，未能赴你设下的小聚：
        🕒 时辰：${n.day} ${n.time}
        📍 地点：${n.place}
        此时已有他约，风起云涌，各安天命。`;
      seal.replyToSender(ctx2, msg2, notice);
    }
...
function sendCombinedDetails(ctx, msg, toRoleName, details) {
    ...
    nodes.unshift({
        ...
        data: {
            name: "关系线档案",
            uin: "10001", 
            content: `📜 角色「${sourceRoleName}」更新了与你的关系细节：`
        }
    });
...
cmd_add_rel_detail.solve = (ctx, msg, cmdArgs) => {
    ...
    if (addr) {
        sendCombinedDetails(ctx, msg, toName, rel.details);
...
// 📖 使用指南指令（精简版）
// ========================
let cmd_user_guide_updated = seal.ext.newCmdItemInfo();
cmd_user_guide_updated.name = "使用指南";
cmd_user_guide_updated.help = "使用指南 [板块名] - 查看系统使用说明\n可用板块：基础/地点/管理/关系/心动/时间\n例如：。使用指南 社交";

cmd_user_guide_updated.solve = (ctx, msg, cmdArgs) => {
    const isAdmin = isUserAdmin(ctx, msg);
    const platform = msg.platform;
    const uid = msg.sender.userId.replace(`${platform}:`, "");
    const section = cmdArgs.getArgN(1)?.toLowerCase();
    ...
    const sections = {
        "基础": () => {
            let sectionText = "📋【基础指令】\n";
            sectionText += "================\n\n";
            sectionText += "👤 角色管理：\n";
            sectionText += "--------------------------------\n";
            sectionText += "• 创建新角色 名称 [群号] - 绑定角色\n";
            sectionText += "• 玩家名单 - 查看所有已绑定的角色\n";
            sectionText += "• 查看天数 - 查看当前全局天数\n";
            sectionText += "• 私约请求 - 查看收到的邀约请求\n";
            sectionText += "• 时间线 - 查看个人日程安排\n";
            sectionText += "• 接受 序号 - 接受请求\n";
            sectionText += "• 拒绝 序号 - 拒绝请求\n";
            sectionText += "• 撤销 [S序号] - 撤回发出的请求\n";
            return sectionText;
        },
        
        "地点": () => {
            let sectionText = "📍【地点系统指令】\n";
            sectionText += "================\n\n";
            sectionText += "玩家可用：\n";
            sectionText += "--------------------------------\n";
            sectionText += "• 查看可私约地点 - 查看可用地点及权限状态\n";
            sectionText += "• 查看我的钥匙 - 查看拥有的地点钥匙\n\n";
            
          if (isAdmin) {
                          sectionText += "🛠️ 地点系统管理（管理员专用）：\n";
                          sectionText += "--------------------------------\n";
                          sectionText += "• .地点管理 添加 地点名:描述 - 新增地点\n";
                          sectionText += "• .地点管理 删除 地点名 - 移除地点及关联钥匙\n";
                          sectionText += "• .地点管理 开关 地点名 - 切换地点上锁/解锁状态\n";
                          sectionText += "• .地点管理 钥匙 角色名 地点名 - 发放或收回该地点的钥匙\n";
                          sectionText += "• .地点管理 清空 - [慎用] 清空所有地点及钥匙数据\n";
                          sectionText += "--------------------------------\n";
                          sectionText += "• .批量设置地点 地点1:描述/地点2:描述 - 快速录入多个地点\n";
                          sectionText += "• .批量发放钥匙 角色1/角色2 地点1/地点2 - 快速授权\n";
                          sectionText += "• .设置默认地点权限 需要钥匙/无需钥匙 - 调整新地点初始状态\n";
                          sectionText += "• .查看地点详情 - 监控所有地点状态、创建者及持有者\n";
                          sectionText += "• .查看钥匙分配 - 统计各地点钥匙分布情况\n";
                      }
        ...
        "关系": () => {
            let sectionText = "🤝【关系线系统指令】\n";
            sectionText += "================\n\n";
            sectionText += "所有用户可用：\n";
            sectionText += "--------------------------------\n";
            sectionText += "• 查看关系线 [对方角色名] - 查看关系线记录\n";
            sectionText += "• 确认关系线 对方角色名 内容 - 确认并记录双方的关系线\n\n";
            
            if (isAdmin) {
                sectionText += "管理员专用：\n";
                sectionText += "--------------------------------\n";
                sectionText += "• 设置强制关系线 角色名A 角色名B 内容 - 设置两人间的强制关系线\n";
                sectionText += "• 删除强制关系线 角色名A 角色名B - 删除强制关系线\n";
                sectionText += "• 移除关系线 角色名A 角色名B [确认码] - 移除两个角色之间的关系线\n";
                sectionText += "• 查看关系线 -all - 查看所有强制关系线\n";
                sectionText += "• 清空关系线 [确认码] - 清空当前平台所有关系线\n\n";
                
                sectionText += "💡 系统参数设置请使用：。系统参数";
            }
            return sectionText;
        },
        ...
        "管理": () => {
            ...
            let sectionText = "👑【系统管理指令】\n";
            sectionText += "================\n\n";
            sectionText += "• 设置天数 D1 - 设置全局天数\n";
            sectionText += "• 设置邀约时间 [时间段1] [时间段2] - 设置允许的邀约时间范围\n";
            sectionText += "• 清空邀约时间 - 清空时间限制\n";
            sectionText += "• 添加群号 群号 - 添加可用群号\n";
            sectionText += "• 查看群号 - 查看所有可用群号\n";
            sectionText += "• 移除群号 群号 - 移除群号\n";
            sectionText += "• 结束私约 - 结束当前群的小群\n";
            sectionText += "• 查看到期群 - 查看1小时内到期的群组\n";
            sectionText += "• 发起官约 D1 14:00-15:00 地点 参与者1/参与者2/... - 自动创建官方约会群组\n";
            sectionText += "• 重置计数 call/private/letter/gift/wish/major/all - 重置计数\n";
            sectionText += "• 查看所有请求 - 查看所有待处理请求\n";
            sectionText += "• 删除请求ID 请求编号 - 删除指定请求\n";
            sectionText += "• 查看他人时间线 角色名 - 查看他人日程\n";
            sectionText += "• 清除玩家 角色名 - 删除角色绑定\n";
            sectionText += "• 管理员列表 - 查看所有管理员\n";
            sectionText += "• 强硬初始化 - 重置所有存储数据\n";
            sectionText += "• 初始化 密码 - 初始化非管理类存储\n";
        ...
        "心动": () => {
            let sectionText = "💌【心动信箱系统】\n";
            sectionText += "================\n\n";
            
            sectionText += "玩家可用：\n";
            sectionText += "--------------------------------\n";
            sectionText += "• 发送心动信 收件人 署名 内容 - 向心动信箱投稿（署名可为「匿名」）\n";
            sectionText += "• 查看我的心动信件 - 查看你今日向心动信箱投递的信件（仅限未送出部分）\n";
            sectionText += "• 撤回我的心动信件 序号 - 撤回你今日尚未送出的信件\n\n";
            
            if (isAdmin) {
                sectionText += "管理员专用：\n";
                sectionText += "--------------------------------\n";
                sectionText += "• 统一送心动信 - 将心动信箱中的信件送达对应角色\n";
                sectionText += "• 信箱统计 - 查看今日心动信箱的投稿内容与统计\n\n";
            ...
            sectionText += "\n💡 说明：\n";
            sectionText += "--------------------------------\n";
            sectionText += "• 心动信箱投稿后不会立即送达\n";
            sectionText += "• 由管理员统一时间段批量送信\n";
            sectionText += "• 每日有投稿次数限制\n";
            sectionText += "• 可在信件送出前撤回";
            return sectionText;
        },
        ...
        "时间": () => {
            ...
            let sectionText = "⏰【时间锁定指令】\n";
            sectionText += "================\n\n";
            sectionText += "• 时间锁定 操作 目标 日期 时间 - 管理角色时间锁定状态\n\n";
        ...
        "系统": () => {
            ...
            let sectionText = "⚙️【系统参数指令】\n";
            sectionText += "================\n\n";
            sectionText += "• 系统参数 - 批量设置所有系统参数\n";
            
            sectionText += "📋 可用参数类别：\n";
            sectionText += "--------------------------------\n";
            sectionText += "1. 功能开关：目击报告、地点系统、信件、礼物等\n";
            sectionText += "2. 系统设置：关系线系统、心愿公告、我的信箱上限等\n";
            sectionText += "3. 群组设置：点歌群、后台群、公告群\n";
            sectionText += "4. 概率设置：寄信混乱效果概率、公开概率等\n";
            sectionText += "5. 限制设置：关系线上限、目击每日上限等\n\n";
            
            sectionText += "💡 使用方式：\n";
            sectionText += "--------------------------------\n";
            sectionText += "输入「。系统参数」查看完整格式\n";
            return sectionText;
        }
    };
    ...
    if (!section) {
        if (isAdmin) {
            // 管理员完整指南（简化版索引）
            guide = "🏛️【长日将尽系统 - 管理员指南】\n";
            guide += "================================\n\n";
            guide += "📚 可用板块（输入：。使用指南 [板块名]）：\n";
            guide += "--------------------------------\n";
            guide += "1. 基础 - 基础指令（所有用户可用）\n";
            guide += "2. 社交 - 社交互动指令\n";
            guide += "3. 地点 - 地点系统指令\n";
            guide += "4. 关系 - 关系线系统指令\n";
            guide += "5. 管理 - 系统管理指令\n";
            ...
            guide += "📊 系统信息：\n";
            guide += "--------------------------------\n";
            guide += `• 当前用户：${roleName || "未绑定角色"}\n`;
            guide += `• 用户ID：${uid}\n`;
            guide += "• 身份：管理员\n";
            guide += "• 指令总数：" + Object.keys(ext.cmdMap).length + " 个\n\n";
            
            guide += "💡 快速使用：\n";
            guide += "--------------------------------\n";
            guide += "• 查看基础指令：。使用指南 基础\n";
            guide += "• 查看社交指令：。使用指南 社交\n";
            guide += "• 查看管理指令：。使用指南 管理\n";
            guide += "• 系统参数设置：。使用指南 系统\n";
            guide += "• 查看当前状态：。系统状态\n";
            ...
            guide += "--------------------------------\n";
            guide += "• 创建新角色 名称 - 绑定角色\n";
            guide += "• 发起邀约 D1 14:00-15:00 地点 对方 - 发起私密邀约\n";
            guide += "• 发送心动信 收件人 署名 内容 - 向心动信箱投稿\n";
            guide += "• 查看关系线 [对方] - 查看关系线记录\n";
            guide += "• 私约请求 - 查看待处理请求\n\n";
            
            guide += "📊 个人信息：\n";
            guide += "--------------------------------\n";
            guide += `• 当前角色：${roleName || "未绑定，请先使用「创建新角色」"}\n`;
            guide += `• 用户ID：${uid}\n\n`;
            
            guide += "💡 使用提示：\n";
            guide += "--------------------------------\n";
            guide += "1. 所有指令以句号开头\n";
            guide += "2. 时间格式为 D1 14:00-16:00\n";
            guide += "3. 多人邀约使用斜杠分隔角色名\n";
            guide += "4. 心动信箱投稿每日有限制\n";
        }
    } else {
        // 显示指定板块
        ...
        if (sectionKey) {
            guide = sections[sectionKey]();
            
            // 添加板块信息头
            const fullGuide = `${guide}\n\n📌 当前板块：${sectionKey}`;
            
            seal.replyToSender(ctx, msg, fullGuide);
            return seal.ext.newCmdExecuteResult(true);
        } else {
            seal.replyToSender(ctx, msg, "⚠️ 未找到指定板块，可用板块：基础/社交/地点/管理/关系/心动/时间/系统");
            return seal.ext.newCmdExecuteResult(true);
        }
    }
    
    seal.replyToSender(ctx, msg, guide);
    return seal.ext.newCmdExecuteResult(true);
};

// 替换原来的使用指南指令
ext.cmdMap["使用指南"] = cmd_user_guide_updated;
...
// 注册三个新指令
ext.cmdMap['基础设置'] = cmd_basic_settings;
ext.cmdMap['高级设置'] = cmd_advanced_settings;
ext.cmdMap['设置信箱上限'] = cmd_set_mailbox_limit;

// 保留原系统参数指令（但修改为提示使用新指令）
let cmd_system_params = seal.ext.newCmdItemInfo();
cmd_system_params.name = '系统参数';
cmd_system_params.help = `系统参数已拆分为三个独立指令：

1. 🎯 基础设置 (.基础设置)
   - 包含礼物、发起邀约、寄信、心愿等基础功能开关
   - 包含关系线系统、群组设置

2. 🔧 高级设置 (.高级设置)
   - 包含目击报告、地点系统、匿名信、后台信件等
   - 包含时间设置、混乱效果、公开概率等高级参数
...
cmd_system_params.solve = function(ctx, msg, argv) {
    const helpText = `📋 系统参数已拆分为三个独立指令：

🎯 基础设置 (.基础设置)
   包含礼物、发起邀约、寄信、心愿、心动信、关系线系统等基础功能
   使用：.基础设置 查看当前设置

🔧 高级设置 (.高级设置)
   包含目击报告、地点系统、匿名信、后台信件、时间设置、混乱效果等
   使用：.高级设置 查看当前设置

📮 设置信箱上限 (.设置信箱上限)
   单独设置我的信箱每日上限
   使用：.设置信箱上限 [数字] 或 .设置信箱上限 查看当前设置

💡 提示：每个指令都支持批量设置，查看时会提供完整的参数模板`;
...
cmd_view_all_wechat.help = "。查看微信群 - 管理员查看所有微信群";
cmd_view_all_wechat.solve = (ctx, msg) => {
    ...
    if (activeGroups.length > 0) {
        ...
        activeGroups.forEach((group, index) => {
            ...
            if (group.participants.length === 2) {
                rep += `   关系：${group.participants[0]} ↔ ${group.participants[1]}\n`;
            }
            rep += `\n`;
        });
    }
    ...
    if (endedGroups.length > 0) {
        ...
        endedGroups.forEach((group, index) => {
            ...
            rep += `   创建者：${group.creator}\n`;
            rep += `   创建时间：${group.created_at}\n`;
            rep += `   结束时间：${group.ended_at}\n`;
            rep += `   结束者：${group.ended_by || "未知"}\n`;
            rep += `   原成员：${group.participants.join("、")}\n`;
            rep += `\n`;
        });
    }
    ...
    rep += `📊 统计信息：\n`;
    rep += `• 活跃微信群：${activeGroups.length} 个\n`;
    rep += `• 已结束微信群：${endedGroups.length} 个\n`;
    rep += `• 唯一微信关系：${uniqueRelations.size} 对\n\n`;

    rep += "💡 管理指令：\n";
    rep += "- 在微信群内使用「。结束微信」可结束该群（需管理员权限）\n";
    rep += "- 使用「。开关微信 开启/关闭」控制功能开关";
